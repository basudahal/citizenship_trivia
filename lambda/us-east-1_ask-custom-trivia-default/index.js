...
function handleUserGuess(userGaveUp, handlerInput) {
  ...
  if (sessionAttributes.currentQuestionIndex === GAME_LENGTH - 1) {
    ...
    speechOutput += speechOutputAnalysis + requestAttributes.t(
      ...
      currentScore.toString() > 16 ? 'Congratulations!' : 'Sorry!',
      currentScore.toString() > 16 ? 'passed' : 'failed'
    );
  ...
  speechOutput += speechOutputAnalysis
    + requestAttributes.t('SCORE_IS_MESSAGE', currentScore.toString(), GAME_LENGTH.toString())
    + repromptText;
...
const languageString = {
  en: {
    translation: {
      ...
      NO_MESSAGE: 'Ok, thanks for playing. We\'ll play another time. Goodbye!',
      ...
      GAME_OVER_MESSAGE: 'You got %s out of %s questions correct. %s You %s the test. Thank you for playing!',
